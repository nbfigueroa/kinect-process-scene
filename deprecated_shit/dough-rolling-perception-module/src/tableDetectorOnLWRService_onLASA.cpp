...
vector<int> dough_D1;
vector<int> dough_D2;
double pixel_area, dough_area, radius_a, radius_b, pixel_m_conv (428);
...
//     if(service_flag){
	ROS_INFO("Attractor Estimation Began...");
      ...
      //string camera_frame = "/head_mount_kinect2_rgb_optical_frame";
	string camera_frame = "/camera_rgb_optical_frame";
      try{	    
...
//      pass.setFilterLimits (0.55, 1.);
      pass.setFilterLimits (-0.75, -0.25);
      ...
      *cloud_plane = *cloud_filtered;
	  ...
	  if (!srv.response.dough_found){
	    ...
	    dough_found = 0;
          ...
          double ellipse_radius_a = radius_a/pixel_m_conv;
          double ellipse_radius_b = radius_b/pixel_m_conv;
          double ellipse_area_m =  M_PI*ellipse_radius_a*ellipse_radius_b;

          ROS_INFO_STREAM("Ellipse Radius a [m]: " << ellipse_radius_a);
          ROS_INFO_STREAM("Ellipse Radius b [m]: " << ellipse_radius_b);
	      ...
	      tf::Vector3 out_dir;
	      	      ...
	      	      //-> Roll Attractor
	      	      //-> Use l-1 Norm to decide on the attractor positioning
	      	      double pos_l1_plane = abs(positive.getY()) + abs(positive.getX());
	      	      double neg_l1_plane = abs(negative.getY()) + abs(negative.getX());
	      	      if (pos_l1_plane > neg_l1_plane){
...
//			reach_corner_.setOrigin(tf::Vector3(roller_off[0]+0.03,roller_off[1],roller_off[2]-0.02));
			//LASA
			reach_corner_.setOrigin(tf::Vector3(roller_off[0]+0.04,roller_off[1],roller_off[2]-0.02));
